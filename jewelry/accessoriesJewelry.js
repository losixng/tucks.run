...
const SAMPLE_PRODUCTS = [
  {
    ...
    instock: true,
    type: "accessoriesJewelry",
    categories: ["matching"],
    filters: ["accessoriesJewelry"]
  }
];
...
function applyProducts(allProducts){
  ...
  const fits = allProducts.filter(p => (p.type || '').toLowerCase() === 'accessoriesJewelry');
  console.log('📦 Filtered for jewelry accessories:', fits.length, 'items');
  ...
  console.log('📦 UI updated | Daily:', allMatching.length, '| Party:', allOccasion.length);
  updateUI();
  applyDeepLinkHighlight();
}
