...
const SAMPLE_PRODUCTS = [
  {
    ...
    type: "hairProducts",
    categories: ["matching"],
    filters: ["hairProducts"]
  }
];

/* ============ STATE & DOM ============ */
const matchingFilters = ["maintenance"];
const occasionFilters = ["treatment"];
...
function applyProducts(allProducts){
  ...
  const fits = allProducts.filter(p => (p.type || '').toLowerCase() === 'hairProducts');
  allMatching = fits.filter(p => (p.categories || []).map(c=>c.toLowerCase()).includes('maintenance'));
  allOccasion = fits.filter(p => (p.categories || []).map(c=>c.toLowerCase()).includes('treatment'));
  updateUI();
}
