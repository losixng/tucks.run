...
/* ============ STATE & DOM ============ */
const matchingFilters = ["face"];
const occasionFilters = ["body"];
...
function applyProducts(allProducts){
  ...
  const fits = allProducts.filter(p => (p.type || '').toLowerCase() === 'skinCare');
  allMatching = fits.filter(p => (p.categories || []).map(c=>c.toLowerCase()).includes('face'));
  allOccasion = fits.filter(p => (p.categories || []).map(c=>c.toLowerCase()).includes('body'));
  updateUI();
}
