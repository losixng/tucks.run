...
const SAMPLE_PRODUCTS = [
  {
    ...
    instock: true,
    type: "accessoriesGadgets",
    categories: ["matching"],
    filters: ["accessoriesGadgets"]
  }
];

/* ============ STATE & DOM ============ */
const matchingFilters = ["new"];
const occasionFilters = ["used"];
...
function applyProducts(allProducts){
  ...
  const fits = allProducts.filter(p => (p.type || '').toLowerCase() === 'accessoriesGadgets');
  console.log('📦 Filtered for gadget accessories:', fits.length, 'items');
  allMatching = fits.filter(p => (p.categories || []).map(c=>c.toLowerCase()).includes('new'));
  allOccasion = fits.filter(p => (p.categories || []).map(c=>c.toLowerCase()).includes('used'));
  console.log('📦 UI updated | New:', allMatching.length, '| Used:', allOccasion.length);
  updateUI();
  applyDeepLinkHighlight();
}
