...
/* ============ STATE & DOM ============ */
const matchingFilters = ["work","elegant","trending"];
...
function applyProducts(allProducts){
  ...
  const fits = allProducts.filter(p => (p.type || '').toLowerCase() === 'gowns');
  console.log('📦 Filtered for gowns:', fits.length, 'items');
...
payNow?.addEventListener('click', async ()=>{
  ...
  const order = {
    ...
    note,
    product: { id: currentItem.id, name: currentItem.name, price: Number(currentItem.price), images: currentItem.images || [], supplierEmail: currentItem.supplierEmail, supplierName: currentItem.supplierName },
    ...
    metadata: { custom_fields: [{ display_name: "Product", variable_name: "product", value: order.product.name }] },
      ...
      (async () => {
        ...
        if (!verifyData.status || verifyData.status !== "success" || !verifyData.data || verifyData.data.amount !== Math.round(order.total * 100)) {
          alert("Payment verification failed! Amount mismatch or invalid reference.");
...
function saveOrderAsHTML(order){
  ...
  <div class="card">
    ...
    <p><strong>Transaction & Misc:</strong> ?${order.transactionFee} (Misc: ?${order.miscFee}, Paystack: ?${order.paystackFee})</p>
    <p><strong>Total:</strong> ?${order.total}</p>
    <p><strong>Seller:</strong> ${order.product.supplierName || 'Marketplace'}</p>
