...
/* ============ STATE & DOM ============ */
const matchingFilters = ["body-con","trending","corporate","casual"];
...
function applyProducts(allProducts){
  ...
  const fits = allProducts.filter(p => (p.type || '').toLowerCase() === 'skirts');
  console.log('📦 Filtered for skirts:', fits.length, 'items');
